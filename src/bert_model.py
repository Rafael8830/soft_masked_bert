...
class BertOnlyMLMHead(nn.Cell):
    def __init__(self, config, param_dict, pretrained):
        super().__init__()
        predictions = BertLMPredictionHead(config)
        if pretrained is True:
            load_param_into_net(predictions, param_dict) # load parameters except cls2.dense.weight and cls2.dense.bias
...
class CLS2(nn.Cell):
    def __init__(self, config):
        ...
        self.decoder = nn.Dense(config.hidden_size, config.vocab_size, bias_init="zeros", has_bias=False)
...
class BertLMPredictionHead(nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.cls1 = BertPredictionHeadTransform(config)
        # The output weights are the same as the input embeddings, but there is
        # an output-only bias for each token.
        self.cls2 = CLS2(config)
        # Need a link between the two variables so that the bias is correctly resized with `resize_token_embeddings`

    def construct(self, hidden_states):
        hidden_states = self.cls1(hidden_states)
        hidden_states = self.cls2(hidden_states)
...
class BertPredictionHeadTransform(nn.Cell):
    ...
    def construct(self, hidden_states):
        ...
        hidden_states = self.gule(hidden_states)
...
class CreateAttentionMaskFromInputMask(nn.Cell):
    ...
    def construct(self, input_mask):
        cast = ops.Cast()
        shape = (self.batch_size, 1, self.seq_length)
        attention_mask = cast(self.reshape(input_mask, shape), mstype.float32)
